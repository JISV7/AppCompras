...
async def add_item(
    ...
    product_check = await db.execute(select(Product).where(Product.barcode == item_in.product_barcode))
    if not product_check.scalars().first():
         raise HTTPException(status_code=404, detail=f"Product {item_in.product_barcode} not found. Scan it first!")

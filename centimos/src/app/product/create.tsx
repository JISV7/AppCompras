...
export default function CreateProductScreen() {
	...
	const handleSave = async () => {
		if (!name.trim() || !brand.trim()) {
			Alert.alert(
				"Falta información",
				"Por favor ingresa el nombre y la marca del producto.",
			);
			return;
		}
		...
		try {
			...
			if (imageUri) {
				...
				if (!finalImageUrl) {
					Alert.alert(
						"Advertencia",
						"No se pudo subir la imagen. El producto se guardará sin foto.",
					);
				}
			}
	...
	return (
		...
		>
			<Stack.Screen
				options={{ title: "Agregar Producto", headerBackTitle: "Atrás" }}
			/>

			<ScrollView
				contentContainerStyle={[
					styles.container,
					{ backgroundColor: bgColor, paddingBottom: insets.bottom + 20 },
				]}
			>
				{/* Photo Section */}
				...
				{/* Brand Input */}
				<Text style={[styles.label, { color: subTextColor }]}>Marca *</Text>
				<TextInput
					style={[styles.input, { backgroundColor: inputBg, color: textColor }]}

...
export default function StoresScreen() {
  ...
  const calculateDistance = (lat1?: number, lon1?: number) => {
    ...
    const a = 
      ...
      Math.cos(userLocation.coords.latitude * Math.PI / 180) * Math.cos(lat1 * Math.PI / 180) * 
  ...
  return (
    <View style={[styles.container, { backgroundColor: bgColor, paddingTop: insets.top }]}>
      ...
      <View style={styles.searchContainer}>
        <View style={[styles.searchBar, { backgroundColor: cardColor }]}>
            ...
            {searchQuery.length > 0 && (
                <TouchableOpacity onPress={() => { setSearchQuery(''); initLocationAndFetch(); }}>
      ...
      <StoreDetailModal
        ...
        store={selectedStore}
        distance={selectedStore ? calculateDistance(selectedStore.latitude, selectedStore.longitude) : null}
        onClose={() => {
...
const styles = StyleSheet.create({
  ...
  iconBox: { 
      width: 50, height: 50, borderRadius: 12, backgroundColor: '#E0F2F1', 
  ...
  distanceBadge: { 
      ...
      backgroundColor: '#E0F2F1', alignSelf: 'flex-start', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4
  },

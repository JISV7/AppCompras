...
export function ListItemSheet({
	...
	const [isPurchasedState, setIsPurchasedState] = useState(
	...
	const fetchComparisons = useCallback(async () => {
		...
		try {
			...
			const data = await getProductPriceComparison(
	...
	}, [visible, item, exchangeRate, fetchComparisons]);
	...
	const handleQtyPress = () => {
		if (isPurchasedState)
			return Alert.alert(
				"Ítem Comprado",
				"Desmarca como comprado para editar.",
			);
		if (onUpdateItem) {
	...
	const handlePricePress = () => {
		if (priceLocked)
			return Alert.alert(
				"Lista Finalizada",
				"No se pueden editar precios en una lista ya finalizada.",
			);
		if (isPurchasedState)
			return Alert.alert(
				"Ítem Comprado",
				"Desmarca como comprado para editar el precio.",
			);
		saveLock.current = false;
	...
	const handleSavePrice = () => {
		...
		if (priceValue === "" || priceValue === "0" || Number.isNaN(price)) {
						...
						>
							<View style={styles.productRow}>
								...
								<View style={styles.info}>
									...
									<Text style={[styles.barcode, { color: subTextColor }]}>
										{item.product_barcode}
									</Text>
								</View>
							</View>
									...
									>
										...
										{isEditingQty ? (
											<TextInput
												...
												value={qtyValue}
												onChangeText={(text) =>
													setQtyValue(text.replace(/[^0-9]/g, ""))
												}
									...
									>
										...
										{isEditingPrice ? (
											...
											<View>
												...
												>
													$
													{parseFloat(priceValue || "0").toLocaleString(
														"en-US",
														{
								...
								!item.store_id && (
									<View style={styles.hintBox}>
										...
										<Text style={styles.hintText}>
											Selecciona una tienda abajo para registrar este precio en
											tu historial comparativo.
										</Text>
									</View>
								)}
							...
							<Text
								style={[
									styles.sectionTitle,
									{ color: textColor, marginTop: 10, marginBottom: 15 },
								]}
							>
							...
							{loadingComparisons ? (
								...
								<View style={styles.emptyBox}>
									<Text style={{ color: subTextColor, textAlign: "center" }}>
										Sin registros en otras tiendas.
									</Text>
								</View>
							)}
